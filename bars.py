...
def get_biggest_bar(bars_dict):
    ...
    for numbers_of_bar in range(len(bars_dict['features']) - 1):
        bar_list = bars_dict['features'][numbers_of_bar]['properties']['Attributes']
...
def get_closest_bar(bars_dict, longitude, latitude):
    ...
    for numbers_of_bar in range(len(bars_dict['features']) - 1):
        bars_coordinates = bars_dict['features'][numbers_of_bar]['geometry']['coordinates']
        sorted_close_bar_list.append(bars_coordinates)
    bar_dict = {}
    for numbers_of_bar in range(len(bars_dict['features'])-1):
        sorted_coords = (sorted_close_bar_list[numbers_of_bar][0] - longitude) ** 2 + \
                        (sorted_close_bar_list[numbers_of_bar][1] - latitude) ** 2
        bar_name = bars_dict['features'][numbers_of_bar]['properties']['Attributes']['Name']
